    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            if (type == 1) {
                ...
                sd.updateOrderStatus(order, value, status, pre, post);
            } else if (type == 3) {
                int catID = sd.getcatID(order);
                sd.updateSize(order, value, status,catID, description);
            }
            response.sendRedirect("SettingDetail?setting_id=" + settingId);
        }
    }
