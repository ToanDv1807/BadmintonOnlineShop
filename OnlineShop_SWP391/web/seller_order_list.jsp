...
<html>
    <head>
        <title>Seller order list - Menu</title>
...
<%@ include file="common/sidebar.jsp" %>
        ...
        <div class="filter-section">
            <form action="SellerOrderList" method="GET" style="margin-left: 23%">
                <input type="text" name="query" placeholder="Search by Order ID or Customer Name" value="${param.query}">
        ...
        <div class="content">
            <div class="orders">
                ...
                <div class="status-buttons" style="text-align: center; margin-top: 20px;">
                    <a href="SellerOrderList" class="status-button"> All </a>
                    <c:forEach var="status" items="${statusList}">
                        <c:if test="${status.status eq 1}">
                            <a href="SellerOrderList?status=${status.statusID}&query=${param.query}&fromDate=${param.fromDate}&toDate=${param.toDate}" class="status-button ${param.status == status.statusID ? 'active' : ''}">
                                ${status.statusName}
                            </a>
                        </c:if>
                    </c:forEach>
                ...
                <table class="order-table">
                    ...
                    <tbody id="orderTableBody">
                        <c:forEach items="${list}" var="listOrder">
                            ...
                            <form action="SellerOrderList" method="POST" style="display: flex; align-items: center; gap: 10px;">
                    ...
                    <c:forEach begin="1" end="${endP}" var="i">
                        <c:choose>
                            <c:when test="${not empty param.query or not empty param.fromDate or not empty param.toDate or not empty param.status}">
                                <a href="SellerOrderList?page=${i}&query=${param.query}&fromDate=${param.fromDate}&toDate=${param.toDate}&status=${param.status}" class="${i == currentPage ? 'page-link active' : 'page-link'}">${i}</a>
                            </c:when>
                            <c:otherwise>
                                <a href="SellerOrderList?page=${i}" class="${i == currentPage ? 'page-link active' : 'page-link'}">${i}</a>
                            </c:otherwise>
                        </c:choose>
                    </c:forEach>
                </div>
