                                ...
                                <form class="updateProductForm" action="EditProductServlet" method="POST" enctype="multipart/form-data">
                                    <div class="update-container">
                                        ...
                                        <div class="left-section">
                                            ...
                                            <c:forEach items="${sessionScope.images}" var="i">
                                                <c:if test="${i.productID == product.productID}">
                                                    <img src="${i.productImgUrl}" alt="Product Image" class="product-image" />
                                                </c:if>
                                            </c:forEach>

                                            <!-- Cập nhật ảnh sản phẩm -->
                                            <label style="font-weight: bold" for="productImage">Update Product Image</label>
                                            <br/>
                                            ...
                                            <div class="sub-images-section">
                                                <label style="font-weight: bold;">Sub Images</label>


                                                </br>
                                                <!-- Thêm hình ảnh phụ mới -->
                                                <label style="font-weight: bold" for="newSubImage">New Sub Image:</label>
                                                <input type="file" id="newSubImage" name="newSubImage" accept="image/*" />

                                                <!-- Nút để gửi các hình ảnh phụ -->
                                                <button type="submit" name="action" value="add" style="font-size: 20px; cursor: pointer;">Add</button>


                                                <div class="sub-images">
                                                    <c:set var="hasSubImages" value="false" /> <!-- Biến để kiểm tra có hình ảnh phụ hay không -->

                                                    <c:forEach items="${sessionScope.subImages}" var="subImage">
                                                        <c:if test="${subImage.productID == product.productID}">
                                                            <!-- Hiển thị hình ảnh phụ -->
                                                            <img src="${subImage.imgUrl}" alt="Sub Image" class="sub-product-image" />

                                                            <!-- Thêm checkbox để xóa hình ảnh phụ -->
                                                            <input type="checkbox" name="deleteSubImageIDs" value="${subImage.subImgID}" /> 

                                                            <!-- File input cho hình ảnh phụ -->
                                                            <input type="file" id="subProductImage_${subImage.subImgID}" name="subProductImage_${subImage.subImgID}" accept="image/*" />
                                                            <input type="hidden" id="subImageID_${subImage.subImgID}" name="subImageID" value="${subImage.subImgID}" />

                                                            <br/><br/><br/>

                                                            <c:set var="hasSubImages" value="true" /> <!-- Đặt biến nếu có hình ảnh phụ -->
                                                        </c:if>
                                                    </c:forEach>

                                                    <!-- Hiện nút Delete chỉ khi có hình ảnh phụ -->
                                                    <c:if test="${hasSubImages}">
                                                        <button type="submit" name="action" value="delete" style="cursor: pointer; background-color: red">Delete</button>
                                                    </c:if>
                                                </div>
                                        ...
                                        <div class="right-section">
                                            ...
                                            <!-- Product Brief Info -->
                                            <label for="productBriefInfo">Product Brief Info</label>
                                            <input type="text" id="productBriefInfo" name="productBriefInfo" value="${product.briefInfo}" />
                                            ...
                                            <div class="form-group">
                                                ...
                                                <input type="number" id="quantity" name="quantity" value="${product.quantity}" class="form-control" readonly="" />
                                            </div>
                                            ...
                                            <table class="product-size-stock-table">
                                                ...
                                                <tbody>
                                                    <c:choose>
                                                        <c:when test="${not empty sessionScope.sizes}">
                                                            <c:forEach items="${sessionScope.sizes}" var="size">
                                                                <c:if test="${size.status==1}">
                                                                    <c:set var="hasStock" value="false" />
                                                                    <c:set var="importPrice" value="${product.importPrice}" /> <!-- Default import price -->

                                                                    <c:forEach items="${sessionScope.productAttributes}" var="productAttribute">
                                                                        <c:if test="${size.sizeID == productAttribute.sizeID && productAttribute.productID == product.productID}">
                                                                            <tr>
                                                                                <td>
                                                                                    <input type="hidden" id="sizeID" name="sizeID" value="${size.sizeID}" />
                                                                                    <input type="text" id="sizeName" name="sizeName" value="${size.sizeName}" class="form-control" readonly />
                                                                                </td>
                                                                                <td>
                                                                                    <input type="number" name="stock_${size.sizeID}" value="${productAttribute.stock}" class="form-control" readonly />
                                                                                </td>
                                                                                <td>
                                                                                    <input type="number" name="hold_${size.sizeID}" value="${productAttribute.hold}" class="form-control" readonly />
                                                                                </td>
                                                                                <td>
                                                                                    <input type="number" id="importPrice_${size.sizeID}" name="importPrice_${size.sizeID}" value="${productAttribute.importPrice != null ? productAttribute.importPrice : 0}" class="form-control" readonly />
                                                                                </td>
                                                                                <td>
                                                                                    <input type="number" id="price_${size.sizeID}" name="price_${size.sizeID}" value="${productAttribute.price != null ? productAttribute.price : 0}" class="form-control" required />
                                                                                </td>
                                                                    ...
                                                                    <!-- Nếu không có thuộc tính nào phù hợp -->
                                                                    <c:if test="${!hasStock}">
                                                                        <tr>
                                                                            <td>${size.sizeName}</td>
                                                                            <td>0</td>
                                                                            <td>0</td>
                                                                            <td>
                                                                                <input type="number" name="importPrice_${size.sizeID}" value="0" class="form-control" readonly="" />
                                                                            </td>
                                                                            <td>
                                                                                <input type="number" name="price_${size.sizeID}" value="0" class="form-control" readonly="" />
                                                                            </td>
                                                                        </tr>
                                                                    </c:if>
                                                                </c:if>
                                            ...
                                            <select id="category" name="category">
                                                ...
                                                <c:forEach items="${sessionScope.categories}" var="sc">
                                                    <option value="${sc.catID}" ${product.catID == sc.catID ? 'selected' : ''}>${sc.catName}</option>
                                                </c:forEach>
                                            </select>
                                            ...
                                            <select id="featured" name="featured">
                                                <option value="1" ${product.featureStatus == 1 ? 'selected' : ''}>Yes</option>
                                                <option value="0" ${product.featureStatus == 0 ? 'selected' : ''}>No</option>
                                            </select>
