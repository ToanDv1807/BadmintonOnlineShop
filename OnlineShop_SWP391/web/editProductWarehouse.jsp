...
<html>
    <head>
        ...
        <style>
            ...
            .modal {
                ...
                width: 100%;
                height: 100%;
                background-color: rgba(0, 0, 0, 0.5); /* Tạo nền tối mờ */
            ...
            .modal-content {
                ...
                padding: 20px;
                border-radius: 8px;
                width: 500px; /* Đặt kích thước modal nhỏ */
                box-shadow: 0 4px 8px rgba(0, 0, 0, 0.1); /* Tạo hiệu ứng đổ bóng */
                animation: modalFadeIn 0.3s ease-in-out; /* Hiệu ứng khi hiện modal */
            }

            /* CSS cho phần footer trong modal */
            .modal-footer {
                display: flex;
                justify-content: flex-end; /* Đặt các nút ở cuối modal */
                margin-top: 20px;
            }
            ...
            /* Bố cục cho phần bên phải */
            .right-section {
                display: flex;
                flex-direction: column; /* Đặt chiều dọc cho các thành phần bên trong */
            ...
            .product-size-stock-table th,
            .product-size-stock-table td {
                border: 1px solid #ccc; /* Đường viền cho ô */
                padding: 10px; /* Padding cho ô */
    ...
    <body>
                        ...
                        <div class="container-fluid">
                            ...
                            <ol class="breadcrumb">
                                <li class="breadcrumb-item">
                                    <a href="WarehouseServlet">Dashboard</a>
                                </li>
                                <li class="breadcrumb-item active">Add Product</li>
                            </ol>
                            <%-- Hiển thị thông báo thành công nếu có --%>
                            <c:if test="${not empty successMessageWare}">
                                <div class="alert alert-success">
                                    ${successMessageWare}
                                </div>
                            </c:if>
                            <c:if test="${not empty errorMessageWare}">
                            ...
                            <div class="card mb-3">
                                ...
                                <div class="update-container">
                                    <div class="left-section">
                                        ...
                                        <label for="productName" style="font-weight: bold; text-align: center">${product.productName}</label>
                                    </div>

                                    <div class="right-section">
                                        <!--                                        <form class="updateProductForm" action="EditProduct" method="POST">
                                                                                   
                                        
                                                                                    <div class="button-group">
                                                                                        <button type="button" class="btn-add" onclick="openAddModal()"><i class="fas fa-plus"></i>Import</button>
                                                                                    </div> 
                                        
                                                                                    <div id="addModal" class="modal">
                                                                                        <div class="modal-content">
                                                                                            <h2>Add New Attribute And Stock</h2>
                                                                                            <br/>
                                                                                            <input type="text" id="AttributeValue" name="AttributeValue" placeholder="Enter new attribute" required>
                                                                                            <br/>
                                                                                            <input type="number" id="newStock" name="newStock" placeholder="Enter stock" required>
                                                                                            <input type="hidden" id="productID" name="productID" value="${product.productID}">
                                                                                            <div class="modal-footer">
                                                                                                <button type="button" class="btn" onclick="closeAddModal()" style="margin-right: 10px">Cancel</button>
                                                                                                <button type="submit" id="action" name="action" value="addAttribute">Add Attribute</button>
                                        ...
                                        <c:if test="${not empty sessionScope.deleteSuccessMessage}">
                                            <div class="alert alert-success">
                                                ${sessionScope.deleteSuccessMessage}
                                                <c:remove var="deleteSuccessMessage" scope="session" />
                                            </div>
                                        </c:if>
                                        <c:if test="${not empty sessionScope.deleteErrorMessage}">
                                            <div class="alert alert-danger">
                                                ${sessionScope.deleteErrorMessage}
                                                <c:remove var="deleteErrorMessage" scope="session" />
                                            </div>
                                        </c:if>
                                        ...
                                        <form class="updateProductForm" action="EditProduct" method="POST">
                                                            ...
                                                            <c:forEach items="${sessionScope.sizes}" var="size">
                                                                ...
                                                                <c:forEach items="${sessionScope.productAttributes}" var="productAttribute">
                                                                    <c:if test="${size.sizeID == productAttribute.sizeID && productAttribute.productID == product.productID}">
                                                                        <tr>
                                                                            ...
                                                                            <td>
                                                                                <input type="number" name="hold" value="${productAttribute.hold}" class="form-control" readonly="" />
                                                                            </td>
                                                                            <td>
                                                                                <input type="number" name="importPrice" value="${productAttribute.importPrice}" class="form-control" />
                                                                            </td>
                                                                        </tr>
                                                                ...
                                                                <c:if test="${!hasStock}">
                                                                    <tr>
                                                                        <td>
                                                                            <input type="hidden" id="sizeID" name="sizeID" value="${size.sizeID}" />
                                                                            <input type="text" name="sizeName" value="${size.sizeName}" class="form-control" readonly="" />
                                                                        </td>
                                                                        <td>
                                                                            <input type="number" name="stock" value="0" class="form-control" />
                                                                        </td>
                                                                        <td>
                                                                   <input type="number" name="hold" value="0" class="form-control" readonly="" />
                                                                        </td>
                                                                        <td>
                                                                            <input type="number" name="importPrice" value="0" class="form-control" /> <!-- Use product import price -->
                                                                        </td>
                                                                    </tr>
                                                                </c:if>
                                                            </c:forEach>
                                            ...
                                            <button type="submit" id="action" name="action" value="updateProduct">Import</button>
                                        </form>
        ...
        <script type="text/javascript">
            function doDeleteAttributeValue(productID, attributeValue) {
                console.log('Attempting to delete attribute with value:', attributeValue);
                if (confirm('Are you sure about deleting attribute value: ' + attributeValue + '?')) {
                    // Ensure the URL correctly includes both productID and attributeValue
                    window.location = 'DeleteProductWarehouse?productID=' + productID + '&attributeValue=' + encodeURIComponent(attributeValue);
                }
            }
        </script>
